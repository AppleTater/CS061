...
.orig x3000
;main code block 
	LD	R7,subroutine_addr	;immediately get addr of subroutine
	JSRR	R7			;and call it

;we now have our inputted number stored in R5.

	ADD	R5,R5,#1	;we add one to the stored value. we now want to 				;output that new value
	LD	R7,print_num_addr
	JSRR	R7
	
	HALT

;main code block data
	subroutine_addr	.FILL	x4000	;subroutine addr
	print_num_addr	.FILL	x5000	;printing the inputted number + 1 in dec
.orig x4000
...
LOOP	
	GETC				;take the first character & print
	OUT
...
END
	...
	RET
;subroutine: take input a decimal number as (max) 5 chars. convert each 
;	stored into its actual value to store its actual value. It is 
;	terminated using newline #10
...
;subroutine 2: print out a number in a register R5
;	R1 will have the counter to print
